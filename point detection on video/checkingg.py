...
# Convert angles list to DataFrame, handling None values
angles_df = pd.DataFrame(angles_list, columns=['Right Knee Angle', 'Left Knee Angle', 'Right Elbow Angle', 'Left Elbow Angle', 
...
# Predict the pose for each frame
predictions = model.predict(angles_df)

# Most common prediction in the video frames
from collections import Counter
pose_prediction = Counter(predictions).most_common(1)[0][0]

print(f"The predicted pose is: {pose_prediction}")
